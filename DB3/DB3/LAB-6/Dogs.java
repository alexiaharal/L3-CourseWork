...
public class Dogs extends JFrame {
	...
	private Dogs() {
		...
		System.out.println("driver loaded. get connection");
		try {
	...
	private void redisplay() {
		...
		try {
			...
			Statement attendanceStmt = conn.createStatement();
			ResultSet attendanceRes = attendanceStmt.executeQuery(
																  "SELECT Attendance.showname,"+
																  " Attendance.opendate"+
																  " FROM Dog, Attendance "+
																  "WHERE Dog.dogid=Attendance.dogid"+
																  " AND Dog.name='"+dogName+"' "+
																  "ORDER BY Attendance.opendate ASC");
			if(attendanceRes.next()){
				appearances="The last show "+dogName+" has attended was "+attendanceRes.getString(1)+" on "+attendanceRes.getString(2);
			}else{
				appearances=dogName+" has not attended any show.";	
			}	
			
		}
		catch(Exception e) {
            doError(e, "Failed to execute attendance query.");
		}
		
		Vector <String> ancestors = getAncestors(dogName,null);
		Vector <String> des = getDescendents(dogName,null);
		Vector <String> dogs = getDogs(owner,null);
		int s=0;//will be reset with every 6 dognames printed
		int tot=0;//to be counting the total dognames printed.
		//this will help to avoid the ',' symbold for the 
		//last dogname.
		String ownsDogs="";
		for (String o : dogs) {
			if (s<5&&tot!=dogs.size()-1){
				ownsDogs=ownsDogs.concat(o+", ");
			}else if((s==5)||(tot==dogs.size()-1)){
				ownsDogs=ownsDogs.concat(o);
			}else{
				s=0;
				ownsDogs=ownsDogs.concat("<br>"+o);
			}
			s++;
			tot++;
		}
    ...
    private Vector<String> getAncestors(String dogname, Vector<String> anc) {
		...
		try {
            ...
            doError(e, "Failed to execute ancestor query in getAncestors");
		}
		return anc;	
    }
    ...
    private Vector<String> getDescendents(String dogname, Vector<String> anc) {
		if (anc == null) anc = new Vector<String>();
		try {
			Statement childStmt = conn.createStatement();
			ResultSet childRes = childStmt.executeQuery("SELECT Dog.name " + 
														"FROM Dog " + 
														"WHERE Dog.mothername='"+dogname+"'"+
														" OR Dog.fathername='"+dogname+"'");
			while(childRes.next()){
				String child=childRes.getString(1);
				if(child!=null){
            ...
            doError(e, "Failed to execute ancestor query in getDescendents");
		}
		return anc;
    }
    
    private Vector<String> getDogs(String ownerName, Vector<String> dogs) {
		if (dogs == null) dogs = new Vector<String>();
		try {
			Statement dogsStmt = conn.createStatement();
			ResultSet dogsRes = dogsStmt.executeQuery("SELECT Dog.name " + 
													  "FROM Dog, Owner " + 
													  "WHERE Dog.ownerid=Owner.ownerid AND Owner.name ='"+ownerName+"'");
			while(dogsRes.next()){
				String dog=dogsRes.getString(1);
				if(dog!=null){
					dogs.add(dog);
				}	
			}
            ...
            doError(e, "Failed to execute ancestor query in getDogs");
		}
		return dogs;	
    }	
}
