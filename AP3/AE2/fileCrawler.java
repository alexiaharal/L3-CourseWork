...
//AE2 specification document section 7.
	...
	public Worker createWorker() {
		return new Worker(workQueue, anotherStructure, lockObject);
	}
	/*
	...
	public static void main(String Arg[]) throws Exception{
		...
		directory = Arg[1];
		crawler.workQueue.put(directory);
		System.out.println(crawler.workQueue.peek());
		Thread t = new Thread(crawler.createWorker());
		t.start();
		t.join();
...
/*		String thstring = System.getenv("CRAWLER_THREADS");
	...
	/*	LinkedList<String> objects = new LinkedList<String>();
		for (int i = fileStart; i < Arg.length; i++) {
			...
			if (ext.compareTo("c") != 0 && ext.compareTo("y") != 0
					&& ext.compareTo("l") != 0) {
